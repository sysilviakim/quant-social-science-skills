...
Instruction modules loaded into context, either globally via
`~/.claude/CLAUDE.md` or per-project via a project `CLAUDE.md`.
...
Rules are not auto-discovered. Copy the file into `~/.claude/rules/` and
reference it from a `CLAUDE.md`:

```markdown
@rules/destructive-command-safety.md
```

Global (`~/.claude/CLAUDE.md`) applies everywhere; a project `CLAUDE.md`
